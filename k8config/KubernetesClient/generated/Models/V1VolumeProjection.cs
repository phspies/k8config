// <auto-generated>
// Code generated by https://github.com/kubernetes-client/csharp/tree/master/gen/KubernetesGenerator
// Changes may cause incorrect behavior and will be lost if the code is
// regenerated.
// </auto-generated>

namespace k8s.Models
{
    using Microsoft.Rest;
    using Newtonsoft.Json;
    using System.Collections.Generic;
    using System.Collections;
    using System.Linq;

    /// <summary>
    /// Projection that may be projected along with other supported volume types
    /// </summary>
    [KubernetesProperty(Description: @"Projection that may be projected along with other supported volume types")]
    public partial class V1VolumeProjection
    {
        /// <summary>
        /// Initializes a new instance of the V1VolumeProjection class.
        /// </summary>
        public V1VolumeProjection()
        {
            CustomInit();
        }

        /// <summary>
        /// Initializes a new instance of the V1VolumeProjection class.
        /// </summary>
        /// <param name="configMap">
        /// information about the configMap data to project
        /// </param>
        /// <param name="downwardAPI">
        /// information about the downwardAPI data to project
        /// </param>
        /// <param name="secret">
        /// information about the secret data to project
        /// </param>
        /// <param name="serviceAccountToken">
        /// information about the serviceAccountToken data to project
        /// </param>
        public V1VolumeProjection(V1ConfigMapProjection configMap = null, V1DownwardAPIProjection downwardAPI = null, V1SecretProjection secret = null, V1ServiceAccountTokenProjection serviceAccountToken = null)
        {
            ConfigMap = configMap;
            DownwardAPI = downwardAPI;
            Secret = secret;
            ServiceAccountToken = serviceAccountToken;
            CustomInit();
        }

        /// <summary>
        /// An initialization method that performs custom operations like setting defaults
        /// </summary>
        partial void CustomInit();

        [JsonProperty(PropertyName = "configMap")]
        [KubernetesProperty(IsRequired: false, Description: @"information about the configMap data to project")]
        public V1ConfigMapProjection ConfigMap { get; set; }

        [JsonProperty(PropertyName = "downwardAPI")]
        [KubernetesProperty(IsRequired: false, Description: @"information about the downwardAPI data to project")]
        public V1DownwardAPIProjection DownwardAPI { get; set; }

        [JsonProperty(PropertyName = "secret")]
        [KubernetesProperty(IsRequired: false, Description: @"information about the secret data to project")]
        public V1SecretProjection Secret { get; set; }

        [JsonProperty(PropertyName = "serviceAccountToken")]
        [KubernetesProperty(IsRequired: false, Description: @"information about the serviceAccountToken data to project")]
        public V1ServiceAccountTokenProjection ServiceAccountToken { get; set; }

        /// <summary>
        /// Validate the object.
        /// </summary>
        /// <exception cref="ValidationException">
        /// Thrown if validation fails
        /// </exception>
        public virtual void Validate()
        {
            ConfigMap?.Validate();
            DownwardAPI?.Validate();
            Secret?.Validate();
            ServiceAccountToken?.Validate();
        }
    }
}
